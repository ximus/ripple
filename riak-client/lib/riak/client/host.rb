module Riak
  class Client
    # A connection to a specific Riak host.
    class Host
      ...
      VALID_OPTIONS = [:protocol, :host, :port, :http_port, :pb_port, :prefix, :mapred, :luwak, :solr, :http_backend, :protobuffs_backend, :ssl, :basic_auth]
      ...
      # Set the protocol of the Riak endpoint.  Value must be in the PROTOCOLS 
      # array.
      ...
      def http
        @http ||= begin
                    klass = Client.const_get("#{@http_backend}Backend")
                    if klass.configured?
                      klass.new(client)
                    else
      ...
      def protobuffs
        @protobuffs ||= begin
                          klass = Client.const_get("#{@protobuffs_backend}ProtobuffsBackend")
                          if klass.configured?
                            klass.new(self)
                          else
      ...
      def inspect
        "#<Riak::Client::Host #{protocol}://#{host}:#{protocol == 'pbc' ? pb_port : http_port}>"
      end
